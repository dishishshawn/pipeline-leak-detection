...
- `data/sample/realtime_training_data.csv`: long-horizon hybrid synthetic training corpus for live scoring
...
That dataset is now generated by a hybrid pipeline rather than a stack of short demo episodes:
- multi-day operational histories with regime shifts (`low_demand`, `nominal`, `high_demand`, `maintenance_bypass`, `startup`)
- leak events shaped by physics-derived priors instead of only hand-written ramps
- nuisance disturbances such as demand spikes, pump wear, valve transients, and sensor drift
- SCADA realism overlays including stale reads, spikes, drift, and held-sample dropouts
...
The generator also emits `data/sample/realtime_training_metadata.csv` with per-event metadata, and the validation helper:

```bash
python scripts/validate_synthetic_data.py
```

writes a structural/training-usefulness report under `reports/synthetic_validation/`.
